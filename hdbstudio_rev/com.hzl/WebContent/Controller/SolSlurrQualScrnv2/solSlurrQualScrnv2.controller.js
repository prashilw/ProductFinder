sap.ui.define([
	...
	return baseController.extend("com.hzl.Controller.SolSlurrQualScrnv2.solSlurrQualScrnv2", {
		 ...
		 * @Models i18n for ResourceModel and viewModel for basic view
		 *         operations
		 * @Method viewSettingInit method for instantiation for view setting
		 *         dialog and initialSettings for user data and role based
		 *         visiblity
		 */
		onInit: function() {
		...
		onSearch: function() {
			...
			// oAjaxHandler.setProperties("QueryTemplate",
			// "SAP_ZN_REC/SOLUTION_SLURRY/QRY/XQRY_SOLUNSLUR_QULTY_DIS");
			oAjaxHandler.setProperties("QueryTemplate", "SAP_ZN_REC/SOLUTION_SLURRY1/QRY/XQRY_SOLUNSLUR_QULTY_DIS");
		...
		/**
		 * @Event press event triggers when view setting icon clicked on table
		 *        header
		 */
		...
		onRowChange: function(oEvent) {
			...
			if(oEvent.getSource().getCustomData().length < 1){
			...
			if (editArr.myEditData.length === 0) {
				...
				for (var i = 0; i < editArr.myEditData.length; i++) {
					if (editArr.myEditData[i].GUID === rowData.getProperty("GUID") && editArr.myEditData[i].EQP_NUM === rowData.getProperty("EQP_NUM")) {
						editArr.myEditData[i].S_DATE = rowData.getProperty("S_DATE");
						editArr.myEditData[i].REMARK = rowData.getProperty("REMARK");
		...
		/**
		 * @Event press event trigger on clicking save button of page not add
		 *        dialog
		 * @Visiblity setting visiblity of save and cancel button
		 * @Model getting the required data into model table getSelected item
		 *        method
		...
		onUpdate: function() {
			...
			var oAjaxHandler = ajaxHandler.getInstance();
			oAjaxHandler.setProperties("QueryTemplate", "SAP_ZN_REC/SOLUTION_SLURRY1/QRY/XQRY_SOLUNSLUR_QULTY_UPDATE");
		...
		successOnUpdate: function(rs) {
			if(rs.Rowsets.Rowset[0].Row[0].QrySuccess === 1){
				...
				oViewData.visiblity.tableFieldEditable = false;
				oViewData.visiblity.updateCancel = false;
				oViewData.visiblity.updateSave = false;   
				this.getView().byId("idEdit").setVisible(true);
				this.getView().byId("idDisplay").setVisible(false)
				this.oViewModel.setData(oViewData);
		...
		/**
		 * @Event press event triggers when import icon clicked on table header
		...
		visiblitySettings: function() {
			...
			case "ZNREC_LAB_ANALYST":
				viewModel.visiblity.updateCancel = false;
				viewModel.visiblity.updateSave = false; 	            	
		...
		handleEdit: function(oEvent){
			var viewModel = this.oViewModel.getData();
			this.userRole = viewModel.userDetails.Rowsets.Rowset[2].Row[0].ROLE;
			...
			this.getView().byId("idDisplay").setVisible(true);
		},
		/**
		 * @Function onclick of Display, changes will be reverted back
		 */
		...
		/**
		 * @Function to validate if Date is within the allowed limit
		 */
		onDateChange: function(oEvent){
			var oSelectedDate = oEvent.getParameter("dateValue");
			var oExpectedDate = new Date();
			var oViewData = this.oViewModel.getData();
			if(this.getView().getModel("tableModel")){
				this.getView().getModel("tableModel").setData("");
			}
			this.getView().byId("SSQS_Table").setNoDataText(this.getText("SSQS_noDataText"));
			oViewData.visiblity.tableFieldEditable = false;
			this.oViewModel.setData(oViewData);
			this.oViewModel.refresh(true);
			switch (this.myRole) {
			case "ZNREC_LAB_ANALYST":
				oExpectedDate.setDate(oExpectedDate.getDate() - 2);
				break;
			case "ZNREC_LAB_SUP":
				oExpectedDate.setDate(oExpectedDate.getDate() - 15);
				break;
			case "ZNREC_LAB_ADMIN":
				oSelectedDate.setYear(9999);
				break;          
			}  
			if(oSelectedDate.getTime() <= oExpectedDate.getTime()){
//				var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();
//				sap.m.MessageBox.alert(oResourceBundle.getText("DateErrorText"));
//				oEvent.getSource().setValue(oExpectedDate);
				this.getView().byId("idEdit").setVisible(false);
			}else{
				this.getView().byId("idEdit").setVisible(true);
			}
